...
```python
for n in range(#number of repeats):
	#commands to repeat
```
...
Guiding Questions
How could we use an if statement to pick up an apple if there is one where Reeborg is standing?

Have the student begin by playing around with picking up the apples as they're positioned in the world. 

Lead the students to the conclusion that without looking at the final world, they'd have no idea where the apples are located. Additionally, have them reflect on the idea that their current solution wouldn't work very well if even one apple changed positions in the world, so it's couldn't be used to solve a slightly different problemof Reeborg collecting apples in a world where the apples are arranged in another order.

Here, we'll aim to introduce conditional statements, specifically the **if statement**.
...
Here, we'll introduce the **while loop**. The while loop is a relative of the for loop and the if statement. Essentially, it says to repeat while some condition is true.
...
How can we decompose the code into smaller functions to make it more organized?
 ...
 - This is a good place to introduce if/else statements if your student is interested
 - Emphasize decomposition more heavily in Hurdle 3 and Hurdle 4
