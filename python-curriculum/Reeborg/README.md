...
**Newspaper 0** - `take()`, `put()`, defining functions like `turn_right()`
...
There are two ways to write a loop that repeats certain lines of code:  
   ...
   ```   
**Around 1** - for loops
* How would you get Reeborg to move along the bottom side of the world with a for loop and `move()`? Now, how would you repeat this action for the remaining sides?  
   
**Hurdle 1** - for loops, defining functions
...
**Around 1 - apple** - if statements, `object_here()`
...
**Around 1 - variable** - while loops, `front_is_clear()`
...
**Hurdle 2** - `at_goal()`, `done()`, `not` in front of a condition
